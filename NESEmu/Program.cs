...
namespace NESEmu
{
    ...
    class Program
    {
        public static Frame show_tile_bank(ref List<byte> chrRom, int _bank) {
            Frame frame = new Frame();
            var tile_y = 0;
            var tile_x = 0;
            var bank = (_bank * 0x1000);
            ...
            for (int tile_n = 0; tile_n <= 255; tile_n++)
            {
                ...
                var tile = chrRom.GetRange(bank+tile_n*16, 16);
                ...
                for (int y = 0; y <= 7; y++)
                {
                    ...
                    for (int x = 7; x >= 0; x--) {
                        ...
                        {
                            case 0:
                                rgb = Pallete.SYSTEM_PALLETE[0x01];
                                break;
                            case 1:
                                rgb = Pallete.SYSTEM_PALLETE[0x23];
                                break;
                            case 2:
                                rgb = Pallete.SYSTEM_PALLETE[0x27];
                                break;
                            case 3:
                                rgb = Pallete.SYSTEM_PALLETE[0x30];
                                break;
                            default:
        ...
        static bool initSDL() {
            ...
            // SDL.SDL_RenderSetScale(gRenderer, 3, 3);
        ...
        public Frame show_tile(ref List<byte> chrRom, int _bank, int tile_n) {
            Frame frame = new Frame();
            int bank = (_bank * 0x1000);

            List<byte> tile = chrRom.GetRange(bank + tile_n * 16, 15);
